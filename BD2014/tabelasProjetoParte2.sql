...
DROP TRIGGER IF EXISTS valor_minimo;

delimiter //
CREATE TRIGGER valor_minimo BEFORE INSERT ON lance
FOR EACH ROW
BEGIN 
	IF  (NEW.valor < ( Select valorbase 
					   From leilaor natural join leilao
					   Where NEW.leilao = lid
					   ))

    THEN Call valor_minimo();
    END IF;

	IF (NEW.valor < ( Select valor 
						   from (select leilao, max(valor) as valor from lance group by leilao) as val
						   Where NEW.leilao = val.leilao))
    THEN Call valor_minimo();
    END IF;
END//
delimiter ;	
